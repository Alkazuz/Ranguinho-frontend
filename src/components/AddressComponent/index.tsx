    ...
    render(): React.ReactNode {
        ...
        const onClickAdress = (e) => {
            if(e.target.className && e.target.className == "address")
                this.setState({open: !((this.state as any) as any).open})
        ...
        return (
            <div className="address-menu" onClick={onClickAdress}>
                ...
                <div id='overlay' onClick={onClickOverlay} style={{opacity: ((this.state as any).open ? 1 : 0 ), display: ((this.state as any).open ? "flex": "absolute"), top: ((this.state as any).open ? 0 : '100%')}}>
                    {   (this.state as any).open && 
                        <div className='address-menu-open lite-shadow' style={{display: ((this.state as any).open ? "flex": "absolute"), top: ((this.state as any).open ? 0 : '100%')}}>
